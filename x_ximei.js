...
本脚本以学习为主！
使用方法: 打开西梅，首页推荐下拉刷新获得数据 / 20 9 * * * 

ximeiurl 抓包 按域名 找到hubonews 带list的请求的  article/list  请求链接即为该数据
ximeihd  抓包请求链接的头部 则为该数据
body为分页数据 脚本默认内置不需要抓包

变量 XMCK  格式 url 换行 header
*/
const $ = new Env('西梅');
...
~(async () => {
  if (process.env.XMCK && process.env.XMCK.split('\n').length > 0) {
    xmckArr = process.env.XMCK.split('\n');
