...
const inviteCodes = [
  `oMZeX7QcoN9VAuZnNbIj-Khl4vsVYyTtM2LD815C8MyOsprd`,
  `u9dqB7oV9tQVVqRkdfw8sziHsAwXQzjKUAbfoKHbDdVD`,
];
!(async () => {
  await requireConfig();
  if (!cookiesArr[0]) {
      ...
      try {
        if (err) {
          ...
          if (data && data.data['bizCode'] === 0) {
            ...
            await $.wait(2000)
            await submitShareCode({ 'shareCode': inviteId ,'pt_key': $.nickName || $.UserName }, 'new');
            await $.wait(2000)
            for (let i = 1; i <= 6; ++i) {
