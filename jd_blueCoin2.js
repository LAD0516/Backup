...
let coinToBeans = '食食有爱' // $.getdata('coinToBeans') || 20; //兑换多少数量的京豆（20或者1000），0表示不兑换，默认兑换20京豆，如需兑换把0改成20或者1000，或者'商品名称'(商品名称放到单引号内)即可
let coinToBeans2 = '三只松鼠坚果大礼包'
let coinToBeans3 = '礼包年货'
...
async function PrizeIndex() {
  ...
  if (`${coinToBeans}` === '1000') {
    ...
    for (let index = 0; index < prizeList.length; index ++) {
      if (prizeList[index].title.indexOf(coinToBeans) > -1 || prizeList[index].title.indexOf(coinToBeans2) > -1 || prizeList[index].title.indexOf(coinToBeans3) > -1) {
